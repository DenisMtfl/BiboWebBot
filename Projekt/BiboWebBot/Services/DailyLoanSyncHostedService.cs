    ...
    ILogger<DailyLoanSyncHostedService> logger) : BackgroundService
{
    ...
    private async Task RunSyncAsync(DailySyncSettings settings, CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                var credentials = new BiboWebBot.VoebbParsing.VoebbCredentials
                {
            ...
            logger.LogInformation(
                "DailySync: Frühestes Datum {DueDate} verarbeitet. MQTT={MqttOk}. Google-Kalender ist in dieser Konfiguration deaktiviert.",
