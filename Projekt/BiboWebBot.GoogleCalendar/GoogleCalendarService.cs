...
public sealed class GoogleCalendarService(IConfiguration configuration, ILogger<GoogleCalendarService> logger, IHttpClientFactory httpClientFactory, IMemoryCache cache) : IGoogleCalendarService
{
    ...
    public async Task<bool> CreateEarliestLoanEventByConsoleLoginAsync(DateOnly dueDate, string? accountLabel, string? eventSummaryTemplate = null, CancellationToken cancellationToken = default)
    {
        var clientId = configuration["Google:ClientId"];
        var clientSecret = configuration["Google:ClientSecret"];
        var tokenStorePath = configuration["Google:TokenStorePath"];
        var redirectUri = configuration["Google:RedirectUri"];
        var calendarId = configuration["Google:CalendarId"];

        if (string.IsNullOrWhiteSpace(clientId) || string.IsNullOrWhiteSpace(clientSecret) || string.IsNullOrWhiteSpace(tokenStorePath) || string.IsNullOrWhiteSpace(calendarId))
        {
            logger.LogWarning("Google OAuth Konfiguration fehlt. Kalendereintrag wird übersprungen.");
            return false;
        }
        ...
        try
        {
            ...
            {
                logger.LogInformation("Google OAuth RedirectUri: {RedirectUri}", redirectUri);
            }
        ...
        {
            logger.LogError(ex, "Google-Kalendereintrag via Console OAuth konnte nicht erstellt werden.");
            return false;
        }
    }
